...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME background.color=$SURFACE0 label.drawing=no
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK}  ${ARTIST}" label.drawing=yes background.color=$SURFACE0

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME background.color=$SURFACE1
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME background.color=$SURFACE1 label.drawing=no
    else
        sketchybar --set $NAME background.color=$SURFACE0
    fi
}
